                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/workinghours/add')">
                                <router-link :to="`/workinghours/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/workinghours'}" :model="pageBreadCrumb" />
                            </template>
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="schedules_label" header="Horario" >
                                            <template #body="{data}">
                                                {{ data.schedules_label }}
                                            </template>
                                        </Column>
                                        <Column  field="day" header="Dia" >
                                            <template #body="{data}">
                                                {{ data.day }}
                                            </template>
                                        </Column>
                                        <Column  field="is_working" header="Trabajando" >
                                            <template #body="{data}">
                                                {{ data.is_working }}
                                            </template>
                                        </Column>
                                        <Column  field="start_time" header="Hora de entrada" >
                                            <template #body="{data}">
                                                {{ data.start_time }}
                                            </template>
                                        </Column>
                                        <Column  field="end_time" header="Hora de salida" >
                                            <template #body="{data}">
                                                {{ data.end_time }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('workinghours/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'workinghourslist',
		},
		apiPath: {
			type: String,
			default: 'workinghours/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/workinghours/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('workinghours/view')
		},
		{
			label: "Edit",
			to: `/workinghours/edit/${data.id}`,
			icon: "pi pi-pencil",
	...
	onMounted(()=>{ 
		const pageTitle = "Horas Trabajadas";
