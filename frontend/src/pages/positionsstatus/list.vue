                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/positionsstatus/add')">
                                <router-link :to="`/positionsstatus/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/positionsstatus'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{data}">
                                                <router-link :to="`/positionsstatus/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('positionsstatus/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'positionsstatuslist',
		},
		apiPath: {
			type: String,
			default: 'positionsstatus/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/positionsstatus/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('positionsstatus/view')
		},
		{
			label: "Edit",
			to: `/positionsstatus/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('positionsstatus/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('positionsstatus/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Positions Status";
