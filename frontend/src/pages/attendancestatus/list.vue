                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/attendancestatus/add')">
                                <router-link :to="`/attendancestatus/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/attendancestatus'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{data}">
                                                <router-link :to="`/attendancestatus/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('attendancestatus/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'attendancestatuslist',
		},
		apiPath: {
			type: String,
			default: 'attendancestatus/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/attendancestatus/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('attendancestatus/view')
		},
		{
			label: "Edit",
			to: `/attendancestatus/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('attendancestatus/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('attendancestatus/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Attendance Status";
