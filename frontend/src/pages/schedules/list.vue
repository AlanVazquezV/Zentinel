                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/schedules/add')">
                                <router-link :to="`/schedules/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/schedules'}" :model="pageBreadCrumb" />
                            </template>
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="schedulesstatus_label" header="Estatus" >
                                            <template #body="{data}">
                                                {{ data.schedulesstatus_label }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('schedules/delete')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/schedules/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('schedules/view')
		},
		{
			label: "Edit",
			to: `/schedules/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('schedules/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('schedules/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Horarios";
