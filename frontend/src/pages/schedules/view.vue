                            ...
                            <div >
                                <div class="mb-3 grid justify-content-start">
                                    ...
                                    <div class="col-12 md:col-4">
                                        <div class="card flex gap-3 align-items-center p-3 ">
                                            <div class="">
                                                <div class="text-400 font-medium mb-1">Schedulesstatus Id</div>
                                                <div class="font-bold">{{ item.schedulesstatus_id }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 md:col-4">
                                        <div class="card flex gap-3 align-items-center p-3 ">
                                            <div class="">
                                                <div class="text-400 font-medium mb-1">Schedulesstatus Label</div>
                                                <div class="font-bold">{{ item.schedulesstatus_label }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'schedulesview',
		},
		apiPath: {
			type: String,
			default: 'schedules/view',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "Edit",
			command: (event) => { app.openPageDialog({ page:'schedules/edit', url: `/schedules/edit/${data.id}`, closeBtn: true }) },
			icon: "pi pi-pencil",
