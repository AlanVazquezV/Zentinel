				...
				<div class="card">
					<div class="mb-4">
						<div class="grid align-items-center justify-content-between">
							...
							<div class="col">
								<div class="text-2xl font-bold">Verificación OTP</div>
							</div>
							<div class="col-3">
								<div class="text-2xl font-bold text-info">{{ countDown }}</div>
							</div>
						</div>
						<div class="text-primary">Se ha enviado OTP a su número de teléfono a través de SMS</div>
					</div>
					<form ref="observer" tag="form" @submit.prevent="startOtpValidation()">
						<div class="grid justify-content-between">
							<div class="col">
								<InputText class="w-full text-center otp-input" placeholder="Ingresar OTP" v-model="otpCode" required="required" type="text" />
							</div>
							<div class="col-3 tetx-right">
								<Button class="p-button-lg" :loading="loading" type="submit" label="Verificar" />
							</div>
						</div>
