			...
			<div class="col md:col-5">
				<div class="card">
					...
					<form ref="observer" tag="form" @submit.prevent="submitForm()">
						<div class="my-3">
							<Password class="w-full" inputClass="w-full" :feedback="true" toggleMask v-model="formData.password" placeholder="Nueva contraseña" :class="getErrorClass('password')" />
							 <small v-if="isFieldValid('password')" class="p-error">{{ getFieldError('password') }}</small>
						</div>
						<div class="my-3">
							<Password class="w-full" inputClass="w-full" :feedback="false" toggleMask v-model="formData.confirm_password" placeholder="Confirmar nueva contraseña"
						...
						<div class="text-center">
							<Button type="submit" :loading="saving" label="Cambia la contraseña" />
						</div>
					</form>
				</div>
			</div>
...
<script setup>
	...
	onMounted(()=>{
		const pageTitle = "Restablecer la contraseña";
