                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/biometricstatus/add')">
                                <router-link :to="`/biometricstatus/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/biometricstatus'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{data}">
                                                <router-link :to="`/biometricstatus/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('biometricstatus/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'biometricstatuslist',
		},
		apiPath: {
			type: String,
			default: 'biometricstatus/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/biometricstatus/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('biometricstatus/view')
		},
		{
			label: "Edit",
			to: `/biometricstatus/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('biometricstatus/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('biometricstatus/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Biometric Status";
