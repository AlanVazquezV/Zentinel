                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/attendance/add')">
                                <router-link :to="`/attendance/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/attendance'}" :model="pageBreadCrumb" />
                            </template>
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="check_type" header="Check Type" >
                                            <template #body="{data}">
                                                {{ data.check_type }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="attendancestatus_label" header="Estatus" >
                                            <template #body="{data}">
                                                {{ data.attendancestatus_label }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('attendance/delete')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/attendance/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('attendance/view')
		},
		{
			label: "Edit",
			to: `/attendance/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('attendance/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('attendance/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Asistencia";
