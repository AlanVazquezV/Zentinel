                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/incidents/add')">
                                <router-link :to="`/incidents/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/incidents'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="id" @sort="onSort($event)" class=" table-hover table-striped" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="justified" header="Justificacion" >
                                            <template #body="{data}">
                                                {{ data.justified }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="documents" header="Documentos" >
                                            <template #body="{data}">
                                                {{ data.documents }}
                                            </template>
                                        </Column>
                                        <Column  field="attendance_date" header="Fecha" >
                                            <template #body="{data}">
                                                {{ data.attendance_date }}
                                            </template>
                                        </Column>
                                        <Column  field="incidentsstatus_label" header="Estatus" >
                                            <template #body="{data}">
                                                {{ data.incidentsstatus_label }}
                                            </template>
                                        </Column>
                                        <Column  field="employees_employee_number" header="No. Empleado" >
                                            <template #body="{data}">
                                                {{ data.employees_employee_number }}
                                            </template>
                                        </Column>
                                        <Column  field="updated_by" header="Actualizado  por" >
                                            <template #body="{data}">
                                                {{ data.updated_by }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('incidents/delete')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/incidents/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('incidents/view')
		},
		{
			label: "Edit",
			to: `/incidents/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('incidents/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('incidents/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Incidencias";
