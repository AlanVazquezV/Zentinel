                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/biometrics/add')">
                                <router-link :to="`/biometrics/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/biometrics'}" :model="pageBreadCrumb" />
                            </template>
                                ...
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{data}">
                                                <router-link :to="`/biometrics/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="biometrictypes_label" header="Tipo" >
                                            <template #body="{data}">
                                                {{ data.biometrictypes_label }}
                                            </template>
                                        </Column>
                                        <Column  field="biometricstatus_label" header="Estatus" >
                                            <template #body="{data}">
                                                {{ data.biometricstatus_label }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('biometrics/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'biometricslist',
		},
		apiPath: {
			type: String,
			default: 'biometrics/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/biometrics/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('biometrics/view')
		},
		{
			label: "Edit",
			to: `/biometrics/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('biometrics/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('biometrics/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Biometricos";
