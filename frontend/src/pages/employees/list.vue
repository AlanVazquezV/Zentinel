                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/employees/add')">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/employees'}" :model="pageBreadCrumb" />
                            </template>
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="employee_number" header="No. Empleado" >
                                            <template #body="{data}">
                                                <router-link :to="`/employees/view/${data.id}`">
                                        ...
                                        <Column  field="surname" header="Apellido Materno" >
                                            <template #body="{data}">
                                                {{ data.surname }}
                                            </template>
                                        </Column>
                                        <Column  field="tax_id" header="RFC" >
                                            <template #body="{data}">
                                                {{ data.tax_id }}
                                            </template>
                                        </Column>
                                        <Column  field="id_number" header="CURP" >
                                            <template #body="{data}">
                                                {{ data.id_number }}
                                            </template>
                                        </Column>
                                        <Column  field="status" header="Estatus" >
                                            <template #body="{data}">
                                                <Button class="p-button-text" icon="pi pi-circle-fill" :label="data.employeestatus_label" v-if="data.status" @click="app.navigateTo(`/employeestatus//${data.status}`)" />
                                            </template>
                                        </Column>
                                        <Column  field="schedules_label" header="Horario asignado" >
                                            <template #body="{data}">
                                                {{ data.schedules_label }}
                                            </template>
                                        </Column>
                                        <Column  field="positions_title" header="Posicion" >
                                            <template #body="{data}">
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('employees/delete')">
...
<script setup>
	...
	function getActionMenuModel(data){
		return [
		{
			...
			icon: "pi pi-eye",
			visible: auth.canView('employees/view')
		},
		{
			label: "Edit",
