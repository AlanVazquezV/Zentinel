                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-12 md:col-3 " >
                            <template v-if="auth.canView('/positions/add')">
                                <router-link :to="`/positions/add`">
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/positions'}" :model="pageBreadCrumb" />
                            </template>
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="title" header="Posicion" >
                                            <template #body="{data}">
                                                {{ data.title }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="positionsstatus_label" header="Estatus" >
                                            <template #body="{data}">
                                                {{ data.positionsstatus_label }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex justify-content-center flex-grow-0">
                                            <template v-if="auth.canView('positions/delete')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/positions/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('positions/view')
		},
		{
			label: "Edit",
			to: `/positions/edit/${data.id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('positions/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: auth.canView('positions/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = "Posiciones";
